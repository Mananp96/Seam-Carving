...
// finding the minimum row or column value Index from energy Map
...
//find the vertical or horizontal seams
vector<int> find_seam(Mat& robustImage, String seam_type /*Seam_Location seam_type*/) {
    ...
    if (seam_type == "VERTICAL") {
	    ...
	    //backtracking
            double minimumValueFromTable = MINIMUM_OF(enrgy_pixelRate1, MINIMUM_OF(enrgy_pixelRate2, enrgy_pixelRate3));
            for(int range = value1; range<=value3; range++) {
                if(minimumValueFromTable == robust_table[i][range]) {
                    minValueIndex = range;
                }
            }
        ...
        for (int j = image_columns - 2; j >= 0; j--) {
            int value1 = MAXIMUM_OF(minValueIndex - 1, 0);
            int value2 = minValueIndex;
            int value3 = MINIMUM_OF(minValueIndex + 1, image_rows - 1);
            double enrgy_pixelRate1 = robust_table[value1][j];
            double enrgy_pixelRate2 = robust_table[value2][j];
            double enrgy_pixelRate3 = robust_table[value3][j];
	    //backtracking
            double minimumValueFromTable = MINIMUM_OF(enrgy_pixelRate1, MINIMUM_OF(enrgy_pixelRate2, enrgy_pixelRate3));
            for(int range = value1; range<=value3; range++) {
                if(minimumValueFromTable == robust_table[range][j]) {
                    minValueIndex = range;
                }
            }
